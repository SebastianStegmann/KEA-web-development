...
$q = $db->prepare(' SELECT user_id, user_name, user_last_name, user_email, user_tag_color, user_is_blocked, user_deleted_at
                    FROM users WHERE user_role_name = "customer" LIMIT :limit OFFSET :offset');
...
if (!isset($_GET['sort'])) {
    ...
    $q = $db->prepare(' SELECT user_id, user_name, user_last_name, user_email, user_tag_color, user_is_blocked, user_deleted_at
    FROM users WHERE user_role_name = "customer" AND user_deleted_at BETWEEN :low AND :high LIMIT :limit OFFSET :offset');
